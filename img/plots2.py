...
def plot_grid_tke(real_h5: str, grid_paths: List[List[Tuple[str,str]]], out_path: str = "grid_tke.png"):
    ...
    # Étiquettes de lignes ($N_{ut}$) sur la marge gauche, sans chevauchement
    for i in range(nrows):
        y_center = (nrows - i - 0.5) / nrows
        fig.text(0.012, y_center, row_names[i],
                 va="center", ha="left", rotation=90, fontsize=10)

    # Super-label des colonnes (H) en haut, dégagé de la légende
    fig.text(0.5, 0.992, r"Inference free-forecast horizon before truth injection ($H$)",
             ha="center", va="top", fontsize=11)

    # Légende unique (au-dessus des axes, sous le super-label) – handles plus visibles
    handles = [line_real_proxy, line_pred_proxy, inj_proxy]
    labels  = ["Simulation", "Prediction", "injections"]
    leg = fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 0.965),
    ...
    # Réserve de l'espace en haut pour super-label + légende (évite toute superposition)
...
if __name__ == "__main__":
    ...
    print("Saved: grid_tke2.png, grid_spectrum.png")
