...
def plot_grid_tke(real_h5: str, grid_paths: List[List[Tuple[str,str]]], out_path: str = "grid_tke.png"):
    ...
    fig.suptitle(r"Inference free-forecast horizon before truth injection ($H$)", y=0.985, fontsize=11)

    # --- Étiquettes de rangées : centrées et décalées à gauche des axes
    fig.canvas.draw()
    # bord gauche réel de la 1re colonne de panneaux
    x_left_axes = min(ax.get_position(fig).x0 for ax in axes[:, 0])
    pad_rows = 0.035  # décalage suffisant pour ne pas toucher la 1re colonne
    x_row_label = x_left_axes - pad_rows
    for i in range(nrows):
        bbox = axes[i, 0].get_position(fig)
        y_center = 0.5 * (bbox.y0 + bbox.y1)
        fig.text(x_row_label, y_center, row_names[i],
...
if __name__ == "__main__":
    plot_grid_tke(REAL_H5, GRID_PATHS, out_path="grid_tke7.png")
    #plot_grid_spectrum(REAL_H5, GRID_PATHS, out_path="grid_spectrum.png")
    print("Saved: grid_tke7.png, grid_spectrum.png")
